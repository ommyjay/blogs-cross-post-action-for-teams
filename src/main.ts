...
async function run(): Promise<void> {
  try {
    ...
    const articlesFiles = await Articles.getArticles(articlesFileLocation) as matterArticle[]
    core.setOutput('formated_articles', articlesFiles[0])

    const postToDevToBlogResponse = await Promise.all(articlesFiles.map(async article => {
    ...
    const postedFileNames = await Articles.getdevToPostedArticleFile(articlesFiles, postToDevToBlogResponse) as string[]
    const devToPrefix = '[dev].'
    const updatedPoestedArticlesFilePaths = await Articles.updatedPostedArticlesFileNames(postedFileNames, devToPrefix)
